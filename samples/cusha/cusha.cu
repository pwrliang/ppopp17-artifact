...
namespace cusha {
    ...
    __global__ void CuSha_GShard(const index_t nshards,
                                 const index_t N,
        ...
        for (index_t offset = threadIdx.x; offset < N; offset += blockDim.x) {
//            assert(offset<)
            local_vertices[offset] = shard_vertex_values[offset];
        }

        __syncthreads();
        for (index_t entry_idx = shard_start_idx + threadIdx.x;
             entry_idx < shard_end_idx; entry_idx += blockDim.x) {
            rank_t new_delta = 0.8 * src_value[entry_idx] / out_degree[entry_idx];
        ...
        int changed = false;
        for (index_t offset = threadIdx.x; offset < N; offset += blockDim.x) {
            if (fabs(local_vertices[offset] - shard_vertex_values[offset]) < 0.00001) {
                changed = true;
                shard_vertex_values[offset] = local_vertices[offset];
            }
        }
...
//                                                               context.host_graph.nnodes);
    const blockSize_N_pair bsizeNPair = {1024, 6144};
    const index_t nshards = std::ceil(
            (double) context.host_graph.nnodes / bsizeNPair.N); //N  The maximum number of vertices inside a shard.
    const index_t nnodes_aligned = nshards * bsizeNPair.N;

//    std::cout << "block size:" << bsizeNPair.blockSize;
//    std::cout << "shard size:" << nshards;
    printf("block size:%d", bsizeNPair.blockSize);
    printf("shard size:%d", nshards);
